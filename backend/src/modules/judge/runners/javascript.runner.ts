...
export class JavaScriptRunner implements CodeRunner {
  async compile(code: string): Promise<CompileResult> {
    ...
    const containerName = createContainerName("javascript-check");
    const args = [...createDockerBaseArgs(containerName, workDir, env.DOCKER_JAVASCRIPT_IMAGE), "node", "--check", "main.js"];
    ...
    if (result.timedOut || isDockerInfrastructureFailure(result)) {
      ...
      return {
        ...
        error: result.timedOut
          ...
          : result.internalError ?? (result.stderr || "Docker failed during JavaScript syntax check."),
    ...
    if (result.exitCode !== 0) {
      ...
      return {
        ok: false,
        error: result.stderr || "JavaScript syntax check failed.",
  ...
  async execute(artifact: RunnerArtifact, testCase: JudgeTestCase): Promise<ExecutionResult> {
    const containerName = createContainerName("javascript-run");
    const args = [...createDockerBaseArgs(containerName, artifact.workDir, env.DOCKER_JAVASCRIPT_IMAGE, true), "node", "main.js"];
    const result = await runDocker(args, {
